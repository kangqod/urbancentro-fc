...
- `pnpm verify` — lint → format:check → typecheck → test:run 을 한 번에 실행하는 SSOT. exit 0 필수.
- `pnpm build` (tsc + vite build)는 `verify`에 포함되지 않는다 — 느려서 훅 밖, 배포/커밋 시점에 별도 실행.
- 브랜드 hex 하드코딩 금지 규칙은 유지되나 자동 게이트에선 제외했다(수동 grep으로 확인). 아래 "SCSS 규칙" 참고.

> 참고: 컴포넌트/RTL 테스트 커버리지가 아직 제한적이라 **렌더링·모션·반응형은 육안 QA가 병행 게이트**다.

## 작업 프로토콜

- **착수 시**: `(범위 / 건드리지 말 것)` 1줄을 먼저 선언한다(예: "범위: guest-modal.tsx 스타일만 / 건드리지 말 것: 밸런싱 로직·zustand 상태").
- **종료 시**: 변경 요약 + `pnpm verify` 실행 결과(pass/fail)를 보고한다. `.claude/hooks/verify-gate.sh`가 Stop 훅으로 동일 명령을 강제하지만, 사람이 직접 커밋하는 경로는 훅 보호 밖이므로 스스로도 보고한다.
